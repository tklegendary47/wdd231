...
function updateFooterInfo() {
    ...
    if (modifiedSpan) {
        ...
        modifiedSpan.textContent = lastModified.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    }
}
...
window.toggleMobileMenu = function() {
    ...
    if (!isActive) { hamburger.classList.add('active'); mobileMenu.classList.add('active'); hamburger.setAttribute('aria-expanded', 'true'); document.body.style.overflow = 'hidden'; }
    else { hamburger.classList.remove('active'); mobileMenu.classList.remove('active'); hamburger.setAttribute('aria-expanded', 'false'); document.body.style.overflow = ''; }
};
window.closeMobileMenu = function() {
    const hamburger = document.querySelector('.hamburger');
    const mobileMenu = document.getElementById('mobileMenu');
    if (!hamburger || !mobileMenu) return;
    hamburger.classList.remove('active'); mobileMenu.classList.remove('active'); hamburger.setAttribute('aria-expanded', 'false'); document.body.style.overflow = '';
};
window.toggleBookmark = function(jobId) { bookmarkedJobs = toggleBookmark(jobId, bookmarkedJobs); saveBookmarks(bookmarkedJobs); renderCurrentPage(); };
...
window.resetFilters = function() { currentFilters = { type: '', location: '', search: '' }; renderCurrentPage(); };
window.showJobDetails = function(jobId) {
    const job = jobsData.find(j => j.id === jobId); if (!job) return;
...
window.closeModal = function() { const modal = document.getElementById('jobModal'); if (modal) { modal.remove(); document.body.style.overflow = ''; } };
window.closeModalOnBackground = function(event) { if (event.target.id === 'jobModal') closeModal(); };

function renderJobCard(job) {
    const isBookmarked = bookmarkedJobs.includes(job.id);
    return `<article class="job-card" onclick="showJobDetails(${job.id})"><button class="bookmark-btn ${isBookmarked ? 'bookmarked' : ''}" onclick="event.stopPropagation();toggleBookmark(${job.id})" aria-label="${isBookmarked ? 'Remove' : 'Bookmark'}">${isBookmarked ? '★' : '☆'}</button><h3 class="job-title">${job.title}</h3><div class="company-name">${job.company}</div><div class="job-meta"><span class="badge">${job.type}</span><span class="badge">📍 ${job.location}</span><span class="badge">💰 ${job.salary}</span></div><p class="job-description">${job.description.substring(0, 120)}...</p></article>`;
}
...
function renderJobsPage() {
    ...
    setTimeout(() => { const ft = document.getElementById('filterType'), fl = document.getElementById('filterLocation'), fs = document.getElementById('filterSearch'); if (ft) ft.value = currentFilters.type; if (fl) fl.value = currentFilters.location; if (fs) fs.value = currentFilters.search; }, 0);
...
function setupEventListeners() {
    document.querySelector('.hamburger')?.addEventListener('click', (e) => { e.stopPropagation(); window.toggleMobileMenu(); });
    document.querySelectorAll('.nav-link').forEach(l => l.addEventListener('click', (e) => { const p = e.target.dataset.page; if (p) navigateTo(p); }));
    document.querySelectorAll('.mobile-menu a').forEach(l => l.addEventListener('click', (e) => { const p = e.target.dataset.page; if (p) navigateTo(p); }));
    document.addEventListener('click', (e) => { if (e.target.classList.contains('cta-button')) { const p = e.target.dataset.nav; if (p) navigateTo(p); } });
    document.querySelector('.slide-nav.prev')?.addEventListener('click', (e) => { e.preventDefault(); changeSlide(-1); });
    document.querySelector('.slide-nav.next')?.addEventListener('click', (e) => { e.preventDefault(); changeSlide(1); });
    document.addEventListener('click', (e) => { const mm = document.getElementById('mobileMenu'), hb = document.querySelector('.hamburger'); if (mm?.classList.contains('active') && !mm.contains(e.target) && !hb?.contains(e.target)) closeMobileMenu(); });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { closeModal(); closeMobileMenu(); } if (e.key === 'ArrowLeft') changeSlide(-1); if (e.key === 'ArrowRight') changeSlide(1); });
}

async function initializeApp() {
    try { console.log('Fetching jobs...'); jobsData = await fetchJobsData(); console.log(`Loaded ${jobsData.length} jobs`); }
    catch (error) { console.error('Init error:', error.message); jobsData = getJobsData(); }
    injectSlideContent(); initSlideshow(); setupEventListeners(); navigateTo('home'); updateFooterInfo();
}
